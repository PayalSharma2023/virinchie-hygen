...
export default function NewBlogPage() {
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'title') {
      set('title', value);
      if (!form.slug) set('slug', value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, ''));
      if (!form.metaTitle) set('metaTitle', value.substring(0, 60));
      return;
    }
    set(name, value);
  };
  ...
  const addBlock = (type: BlockType) =>
    setContentBlocks((prev) => [...prev, defaultBlock(type)]);
  ...
  const updateListItem = (bi: number, li: number, value: string) =>
    setContentBlocks((prev) => prev.map((b, idx) => {
      if (idx !== bi) return b;
      const items = [...(b.items ?? [])];
      items[li] = value;
      return { ...b, items };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedFile) { toast.error('Please select a featured image'); return; }
    ...
    try {
      ...
      const payload = {
        ...
        readTime:     form.readTime || estimateReadTime(contentBlocks),
      };
  ...
  return (
    <div className="max-w-5xl">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-slate-100 mb-8">Create New Blog</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className={cardCls}>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input name="title" value={form.title} onChange={handleInput} required className={inputCls} placeholder="Blog title" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelCls}>Slug *</label>
                <input name="slug" value={form.slug} onChange={handleInput} required className={inputCls} placeholder="url-slug" />
                <p className={hintCls}>/blogs/{form.slug || 'your-slug'}</p>
              </div>
              <div>
                <label className={labelCls}>Category *</label>
                <input name="category" value={form.category} onChange={handleInput} required className={inputCls} placeholder="Hydrology, GIS, Environment…" />
              </div>
            </div>
            <div>
              <label className={labelCls}>Excerpt * <span className="font-normal text-gray-400">({form.excerpt.length}/300)</span></label>
              <textarea name="excerpt" value={form.excerpt} onChange={handleInput} required maxLength={300} rows={3} className={`${inputCls} resize-none`} placeholder="Short description shown in listings" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelCls}>Author</label>
                <input name="author" value={form.author} onChange={handleInput} className={inputCls} placeholder="Er. Vikram Nanda" />
              </div>
              <div>
                <label className={labelCls}>Author Role</label>
                <input name="authorRole" value={form.authorRole} onChange={handleInput} className={inputCls} placeholder="Senior Hydrologist" />
              </div>
            </div>
            <div>
              <label className={labelCls}>Author Bio</label>
              <textarea name="authorBio" value={form.authorBio} onChange={handleInput} rows={2} className={`${inputCls} resize-none`} placeholder="Brief author description shown at end of article" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelCls}>Read Time</label>
                <input name="readTime" value={form.readTime} onChange={handleInput} className={inputCls} placeholder="auto-calculated if blank" />
                <p className={hintCls}>Leave blank to auto-calculate</p>
              </div>
              <div>
            ...
            <div>
              ...
              <input name="tags" value={form.tags} onChange={handleInput} className={inputCls} placeholder="HEC-RAS, Flood Modelling, Himachal Pradesh (comma-separated)" />
            </div>
        ...
        {/* ── Cover Colour ──────────────────────────────────────────────── */}
        <div className={cardCls}>
          <h2 className={cardHeadCls}>Cover Colour <span className="text-sm font-normal text-gray-400">(used as fallback when image unavailable)</span></h2>
          ...
          {imagePreview ? (
            <div className="relative">
              ...
              <p className="text-xs text-center text-slate-500 dark:text-slate-400 mt-2">
                Will upload to Cloudinary when you click &quot;Create Blog&quot;
              </p>
            </div>
          ) : (
            <label className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-gray-300 dark:border-slate-600 rounded-lg cursor-pointer hover:border-sky-500 transition-colors bg-gray-50 dark:bg-slate-700/50">
              ...
              <p className="text-sm text-gray-500 dark:text-slate-400"><span className="font-semibold">Click to select</span> or drag and drop</p>
        ...
        <div className={cardCls}>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input name="metaTitle" value={form.metaTitle} onChange={handleInput} maxLength={60} className={inputCls} placeholder="SEO title" />
            </div>
            <div>
        ...
        <div className={cardCls}>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={form.published}
              ...
              className="w-5 h-5 accent-sky-600 rounded" />
            <span className="text-gray-700 dark:text-slate-300 font-medium">Publish immediately</span>
          </label>
        </div>
        ...
        <div className="flex gap-4">
          <button type="submit" disabled={loading}
            ...
            {loading ? <><Spinner /> Creating…</> : 'Create Blog'}
          </button>
...
function ContentBlockEditor({ block, index, onUpdate, onRemove, onUpdateItem, onAddItem, onRemoveItem }: {
  ...
  const inputCls = "w-full px-3 py-2 border border-gray-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-sky-500 outline-none";
    ...
    <div className="border border-gray-200 dark:border-slate-600 rounded-xl p-4 bg-gray-50/50 dark:bg-slate-800/50 group">
      ...
      {(block.type === 'paragraph') && (
        <textarea value={block.text ?? ''} onChange={(e) => onUpdate({ text: e.target.value })}
          rows={4} className={`${inputCls} resize-none`} placeholder="Paragraph text…" />
      )}

      {(block.type === 'heading' || block.type === 'subheading') && (
        <input value={block.text ?? ''} onChange={(e) => onUpdate({ text: e.target.value })}
          className={`${inputCls} ${block.type === 'heading' ? 'font-bold text-lg' : 'font-semibold'}`}
          ...
          {(block.items ?? []).map((item, li) => (
            <div key={li} className="flex gap-2 items-center">
              ...
              <input value={item} onChange={(e) => onUpdateItem(li, e.target.value)}
                className={`${inputCls} flex-1`} placeholder={`Item ${li + 1}`} />
      ...
      {block.type === 'callout' && (
        <div className="space-y-3">
          <div className="flex gap-3">
            <div className="flex-1">
              <input value={block.title ?? ''} onChange={(e) => onUpdate({ title: e.target.value })}
                className={inputCls} placeholder="Callout title (e.g. Key Insight, Warning…)" />
            </div>
          ...
          <textarea value={block.text ?? ''} onChange={(e) => onUpdate({ text: e.target.value })}
            rows={3} className={`${inputCls} resize-none`} placeholder="Callout body text…" />
        </div>
      )}
    </div>
  );
