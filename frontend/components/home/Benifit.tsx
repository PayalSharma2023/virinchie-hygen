...
export default function BenefitsSection() {
  return (
    <section className="relative py-24 px-6 bg-white dark:bg-slate-950 overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[200px] bg-sky-50 dark:bg-sky-900/10 rounded-full blur-[80px] pointer-events-none" />
      ...
      <div className="relative max-w-5xl mx-auto">
        <div className="text-center mb-14">
          ...
          >
            ...
            <span className="relative inline-block">
              <span className="text-sky-500 dark:text-sky-400">Approach</span>
        ...
        <div className="grid sm:grid-cols-2 gap-5">
          {benefits.map((b, i) => (
            <div
              key={i}
              className={`group flex gap-4 items-start ${b.bg} border ${b.border} rounded-2xl p-6 hover:shadow-md dark:hover:shadow-black/40 hover:-translate-y-0.5 transition-all duration-300 overflow-hidden relative`}
            >
              <div className={`flex-shrink-0 inline-flex items-center justify-center w-10 h-10 rounded-xl ${b.iconBg} shadow-sm ${b.text} group-hover:scale-110 transition-transform duration-300`}>
                {b.icon}
              </div>
              <div>
                <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-1">{b.title}</h3>
